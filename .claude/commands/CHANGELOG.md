...
### Changed（202604-IMS の改訂履歴・ジャンプリンク新設で確立した手順を定型化。Yamazaki-OS/Core/make-report.md にも同内容を同期）
- make-report.md：追記マーカーの直前に HTML アンカー `<a id="add-YYYYMMDD-アカウント名"></a>` を置くルールを追加（見出しの自動アンカーは日本語・全角記号でスラッグが崩れるため明示的アンカーを使う）
- make-report.md：「冒頭の改訂履歴（必須）」節を新設。`> 作成日：` 行の直後に改訂履歴表（日付・追記者・ジャンプリンク）を置き、追記のたびに1行追加する。表がない既存レポートは既存追記マーカーを洗い出してアンカーを補い表を新設する。最終更新日と表の最新行の日付を同時に更新する
