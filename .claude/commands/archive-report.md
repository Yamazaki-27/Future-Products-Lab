...
必ず `find_real_last_edit()`（★/◎/△バッジ列で定義したものと同一関数）を先に試し、実質的な編集コミットが見つかった場合はそのタイムスタンプを使う。見つからない場合（そのファイルの全履歴が一括操作のみ）に限り、`git log --follow -1` の生タイムスタンプにフォールバックする（他に手がかりが無いための次善策であり、参考値であることを利用者は理解しておく）。
...
bash での実装例（`find_real_last_edit()` は前述のものを使う）：

```bash
rel_time() {
  local f="$1"
  local t; t=$(find_real_last_edit "$f")
...
`find_real_last_edit()` の定義は本ファイル内で「★/◎/△バッジ列」節よりも先に読まれるとは限らないため、実装時は関数定義の順序に注意する（`rel_time()` より先に `find_real_last_edit()` を定義する）。
