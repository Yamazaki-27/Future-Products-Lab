...
if [ ! -f PUBLISH_SUMMARY.md ]; then
  ...
  # PUBLISH_SUMMARY.md より新しいファイルを「全件」取得する（差分ビルドの入力になる）
  find . \
    \( -name "Report.md" \
    -o -name "*.txt" \
    -o -path "*/Images/*" \) \
    -newer PUBLISH_SUMMARY.md 2>/dev/null
  # 出力が空なら「変更なし」、1件以上あれば「変更あり」
  # 出力された一覧を「変更ファイルリスト」として保持し、以降の工程で使う
fi
```
...
- `PUBLISH_SUMMARY.md` が存在しない（初回ビルド）→ 通常通り全工程を実行する
- `PUBLISH_SUMMARY.md` が存在し、変更ファイルがある（再ビルド）→ 変更ファイルリストをログに残し、**差分ビルドモード**（次章）で実行する

---

## 差分ビルドモード（トークン消費の最小化）

再ビルド（追記モード）では、変更ファイルリストに載っているファイル**だけ**を処理対象とする。
前回ビルドで確定済みの内容を再点検しない。これがトークン消費を最小にする唯一の方法である。

### やること（変更分のみ）

| 対象 | 処理 |
|---|---|
| 新規・変更された写真 | Read で目視確認 → リサイズ → 採否判断 → 配置 |
| 新規・変更された動画 | サムネイル抽出 → 本体削除 |
| 変更された日報（*.txt） | 全文を読み、**Report.md に未反映の部分だけ**を特定して追記 |
| Report.md | 追記ブロックを末尾に追加（既存本文は変更しない） |
| review / publish の確認 | **追記ブロックと新規写真のみ**を対象とする |

### やらないこと（確定済みの再処理）

- 既存の採用写真・OtherPictures・unUsed を Read で開き直さない（前回ビルドで確認済み）
- Report.md の既存部分を頭から読み直さない。読むのは「章構成の把握に必要な見出し行（grep）」と「追記位置の前後」だけ
- 既存の画像リンク全数チェックをしない。リンク確認は**追記ブロック内のリンクのみ**
- 既存キャプション・本文の校正をやり直さない
- README.md の写真枚数・容量は再集計する（軽い find コマンド1回で済むため）

### Report.md の読み方

```bash
# 全文を Read しない。まず構造だけ把握する
grep -n "^#\|^##\|^###" Report.md   # 章構成
tail -30 Report.md                   # 追記位置の確認
```

追記モードでの既存 Report.md の Read は、上記で足りない場合の**部分読み（offset/limit 指定）**に限る。

### 例外（差分ビルドでも全体を見るケース）

- 追記した写真が既存の章の時系列の**途中**に入るべき場合 → その章だけ部分読みして挿入位置を決める
- publish-report で Markdown 構文エラーが検出された場合 → エラー箇所の前後のみ部分読みして修正する
...
ただし差分ビルドモードでは、審査対象は**変更ファイルリストに載っている新規・変更ファイルのみ**とする（確定済み写真の再審査はしない）。
