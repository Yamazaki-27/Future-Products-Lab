...
README.md の「出張報告書」テーブル・「Strategy」テーブルの各行に、実質的な更新が**過去7日以内**にあったことを示す🆕バッジを付ける。今回処理中のレポートだけでなく、**両テーブルの全行を毎回洗い直す**（stale化防止のため）。凡例がなければテーブル直前に `> 🆕 = 過去7日以内に実質的な更新があったもの` を追加する。
...
- 7日以内 → 該当行のリンクの直後（`]` の後、次の `|` の前）に半角スペース＋`🆕`を付ける。既に付いていれば何もしない
- 7日を超えた、または `PUBLISH_SUMMARY.md` が存在しない → バッジを外す（付いていれば削除する）
...
- 判定・付与ルールは出張報告書テーブルと同じ（7日以内→付与、それ以外→外す）
