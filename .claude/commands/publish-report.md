...
目安は横幅1600px程度とする（PDF化時にA4幅で約220dpiを確保するため）。

- 横幅が極端に大きい場合は、1600〜2000px程度へ縮小する。800px以下へは縮小しないこと（PDFでぼやけるため）。
...
### 9.5 Report.md冒頭の最終更新日をPUBLISH_SUMMARY.mdと同期する

Report.md のH1直後には `> 作成日：YYYY-MM-DD　最終更新日：YYYY-MM-DD` の行がある（make-report.md が記入）。

本スキルの実行日時（12章で `PUBLISH_SUMMARY.md` に書き出す実行日時）と、Report.md側の最終更新日を必ず一致させる。

- 12章で `PUBLISH_SUMMARY.md` に実行日時を書き出す際、同じ日付で Report.md 冒頭の `> 作成日：` 行の「最終更新日」部分だけを上書きする（作成日は変更しない）
- これにより、「出張報告書」テーブルのバッジ判定基準（PUBLISH_SUMMARY.md）と、Report.md を開いたときに見える日付が常に同一になり、バッジの信憑性を裏付ける
