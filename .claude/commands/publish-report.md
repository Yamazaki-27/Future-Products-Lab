...
### 9.4 ★/◎/△バッジの自動更新（過去24時間／3日／7日以内の実質更新）
...
列見出しは空欄でよい（`|  |` とし、ヘッダー行も `|:---:|` とする）。今回処理中のレポートだけでなく、**両テーブルの全行を毎回洗い直す**（stale化防止のため）。凡例がなければテーブル直前に `> ★ = 過去24時間以内に実質的な更新があったもの　◎ = 過去3日以内（24時間超）に実質的な更新があったもの　△ = 過去7日以内（3日超）に実質的な更新があったもの` を追加する。
...
| ◎ | 2026年4月17日 | [🇺🇸 米国／BIC...](Reports/202604-BIC/Report.md) | 山崎・橋本GM | 28枚・4.0MB | [7/8](Reports/archive_log.md) |
...
- 24時間以内 → 該当行の一番左の列（専用列）を `★` にする
- 24時間超・3日以内 → `◎` にする
- 3日超・7日以内 → `△` にする
- 7日を超えた、または `PUBLISH_SUMMARY.md` が存在しない → 該当列を空欄にする

（`PUBLISH_SUMMARY.md` の実行日時は日付単位の記録のため、24時間判定の厳密な時刻比較ができない場合は「実行日が本日と同一なら★、それ以外は日数で判定」のように運用してよい。より厳密に扱いたい場合は実行日時を `YYYY-MM-DD HH:MM` のままUNIX秒に変換して比較する。）
...
- 判定・付与ルールは出張報告書テーブルと同じ（専用列に24時間以内→`★`、24時間超・3日以内→`◎`、3日超・7日以内→`△`、それ以外→空欄）
