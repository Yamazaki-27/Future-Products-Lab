...
### 9.4 🆕バッジの自動更新（過去7日以内の実質更新）

README.md の「出張報告書」テーブル・「Strategy」テーブルに、実質的な更新が**過去7日以内**にあった行を示す🆕バッジを付ける。**表の一番左に専用列を追加し**、該当する行だけ `🆕`、それ以外は空セルにする（リンクの後ろに文字として埋め込むと、行によってリンクの長さが違うためバッジの位置が揃わず見づらい。専用列にすることで縦に並び、一目で分かる）。

列見出しは空欄でよい（`|  |` とし、ヘッダー行も `|:---:|` とする）。今回処理中のレポートだけでなく、**両テーブルの全行を毎回洗い直す**（stale化防止のため）。凡例がなければテーブル直前に `> 🆕 = 過去7日以内に実質的な更新があったもの` を追加する。
...
このリポジトリでは過去に733ファイル・128ファイルを同時に変更する一括リファクタリングコミットがあり、単純に「そのファイルを最後に触ったコミット」を使うと、内容と無関係にほぼ全ファイルが「最近更新」と誤判定される。これを避けるため、**同時に変更されたファイル数が多いコミットは機械的操作とみなして除外**する（コミットメッセージの文字列マッチではなく、変更ファイル数という客観的な指標を使うことで、将来また同種の一括操作が発生しても自動的に対応できる）。

```bash
today=$(date +%s)
find_real_last_edit() {
  local f="$1"
  local threshold=10   # 同時変更ファイル数がこれを超えるコミットは一括操作とみなし除外
...
- 判定・付与ルールは出張報告書テーブルと同じ（専用列に7日以内→`🆕`、それ以外→空欄）
- 将来また大規模なフォルダ整理・一括置換を行う場合は、そのコミットが上記の閾値（10ファイル）に収まっているか確認すること。収まらなければ自動的に除外対象になる

**既知の限界**：この判定はファイル数のみを見ており、「1ファイルだけを移動しただけで内容変更ゼロ」のコミットは除外できない（閾値10以下に収まるため）。日本語ファイル名混在下では `git show --numstat` の行変更量チェックを組み合わせるとかえって不安定になることを確認済みなので、あえて行っていない。数日程度バッジが早めに付く程度のズレは許容する。
